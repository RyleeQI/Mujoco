...
def eval_refinepolicy(policy, env_name, seed, mean, std, seed_offset=100, eval_episodes=10):
	eval_env = gym.make(env_name)
	eval_env.seed(seed + seed_offset)
	...
	for _ in range(eval_episodes):
		...
		while not done:
			state = (np.array(state).reshape(1,-1) - mean)/std
			action = policy.select_refineaction(state)
			state, reward, done, _ = eval_env.step(action)
			avg_reward += reward
...
if __name__ == "__main__":
	...
	# Experiment
	parser.add_argument("--policy", default="Refine_TD3_BC")               # Policy name
	...
	# refine model
	parser.add_argument("--refine_times", type=int, default=1)
	args = parser.parse_args()
	print(args)
	
	tag_name = f"{args.policy}_{args.tag}_refine_times{args.refine_times}_seed{args.seed}"
	if args.en_wandb == 1:
	...
	file_name = f"{args.policy}_{str_env}_{args.refine_times}_{args.seed}"
	print("---------------------------------------")
	...
	# Initialize policy
	policy = model.Refine_TD3_BC(**kwargs)
	...
	if args.en_wandb == 1:
		# pass
		wandb.watch(policy.refineactor, log=all, log_freq=args.eval_freq)
	...
	for t in tqdm(range(int(args.max_timesteps))):
		...
		if (t + 1) % args.eval_freq == 0:
			...
			if stage == 0:
				...
				evaluations.append(eval_refinepolicy(policy, args.env, args.seed, mean, std))
			...
			if args.save_model: 
				...
				if args.en_wandb == 1:
					...
					wandb.save(f"{log_dir}/{file_name}"+ "_refineactor")
					wandb.save(f"{log_dir}/{file_name}"+ "_refineactor_optimizer")
