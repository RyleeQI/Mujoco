...
def eval_random_action_policy(policy, env_name, seed, mean, std, seed_offset=100, eval_episodes=10):
	eval_env = gym.make(env_name)
	eval_env.seed(seed + seed_offset)
...
if __name__ == "__main__":
	...
	# Experiment
	parser.add_argument("--policy", default="Res_TD3_BC")               # Policy name
	...
	tag_name = f"{args.policy}_{args.tag}_seed{args.seed}"
	if args.en_wandb == 1:
	...
	for t in tqdm(range(int(args.max_timesteps))):
		...
		if (t + 1) % args.eval_freq == 0:
			...
			if stage == 0:
				...
				evaluations_random_action.append(eval_policy(policy, args.env, args.seed, mean, std))
			elif stage == 1:
				evaluations.append(eval_respolicy(policy, args.env, args.seed, mean, std))
				evaluations_random_action.append(eval_random_action_policy(policy, args.env, args.seed, mean, std))
			...
			if args.en_wandb == 1:
					wandb.log({
						...
						"Test Score random_action": evaluations_random_action[-1],
						"Test Score delta": evaluations[-1] - evaluations_random_action[-1],
						"Average Test Score": np.mean(evaluations[-10:]),
						"Average Test Score random_action": np.mean(evaluations_random_action[-10:]),
						"Average Test Score delta": np.mean(evaluations[-10:]) - np.mean(evaluations_random_action[-10:]),
